...
public sealed class RelationalIntegrationTests : IClassFixture<RelationalIntegrationFixture>, IDisposable
{
    ...
    private RelationalIntegrationFixture.Target Use(DbProviderKind provider)
    {
        ...
        _repo = new EfRepository(_db, Options.Create(new AgrumySettings()), NullLogger<EfRepository>.Instance, new NullCache(), new EfAuditLogRepository(_db), new EfRefreshTokenRepository(_db), new EfControllerDataRepository(_db), new EfDiscoveryRepository(_db), new EfTenantRepository(_db));
        return t;
    }
    ...
    public async Task RefreshToken_ConcurrentRotateOfSameToken_OnlyOneWins(DbProviderKind provider)
    {
        ...
        var repoA = new EfRepository(dbA, Options.Create(new AgrumySettings()), NullLogger<EfRepository>.Instance, new NullCache(), new EfAuditLogRepository(dbA), new EfRefreshTokenRepository(dbA), new EfControllerDataRepository(dbA), new EfDiscoveryRepository(dbA), new EfTenantRepository(dbA));
        var repoB = new EfRepository(dbB, Options.Create(new AgrumySettings()), NullLogger<EfRepository>.Instance, new NullCache(), new EfAuditLogRepository(dbB), new EfRefreshTokenRepository(dbB), new EfControllerDataRepository(dbB), new EfDiscoveryRepository(dbB), new EfTenantRepository(dbB));
